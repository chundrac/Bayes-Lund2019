...
We use Hamiltonian Monte-Carlo as implemented in tensorflow probability (tfp).
We transform unconstrained parameters into vl
Tfp runs several chains in parallel; hence, each parameter is really an array of C parameters
"""
...
def initialize_unconstrained_params(K,L,S_u):
    ...
    return(alpha_u,theta_u,phi_u)


def transform_params(alpha_u,theta_u,phi_u,X,part_u):
    ...
    return(alpha_c,theta_c,phi_c)


def joint_log_prob(lang_array,feat_var_array,K,L,R,X,part,part_u,alpha_u,theta_u,phi_u):
    alpha_c,theta_c,phi_c = transform_params(alpha_u,theta_u,phi_u,X,part_u)
    #define the priors
    theta = tfd.Dirichlet(tf.ones([L,K])*alpha_c)
    phi = [tfd.Dirichlet(tf.ones([R[x]])*lambda_) for x in range(X)]
    #compute log priors
...
def run_inference(K,chains):
    ...
    for c in range(chains):
        ...
        alpha_u,theta_u,phi_u = initialize_unconstrained_params(K,L,S_u)
        def target_log_prob_fn(alpha_u,theta_u,phi_u):
            return(joint_log_prob(lang_inds,feat_var_inds,K,L,R,X,part,part_u,alpha_u=alpha_u,theta_u=theta_u,phi_u=phi_u))
        ...
        states, kernel_results = tfp.mcmc.sample_chain(
            ...
            current_state=[alpha_u,theta_u,phi_u],
            kernel=kernel)
    ...
    f = open('posterior_sparse_{}.pkl'.format(K),'wb')
    pkl.dump(posts,f)
    f.close()
...
def main():
    ...
    try:
        ...
        print('usage: python run_K_sparse.py [n clusters]')
    K = int(sys.argv[1])
    run_inference(K,4)
